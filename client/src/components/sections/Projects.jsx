...
var ProjectItem = function (_a) {
    var title = _a.title, subtitle = _a.subtitle, date = _a.date, achievement = _a.achievement, technologies = _a.technologies, points = _a.points;
    return (<AnimatedCard>
    <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-6">
      <div>
        <div className="flex items-center mb-2">
          <h3 className="text-xl font-bold mr-3">{title}</h3>
          {achievement && (<span className="bg-accent bg-opacity-20 text-accent px-2 py-1 rounded text-xs font-medium">
              {achievement}
            </span>)}
        </div>
    ...
    <div className="mb-4">
      <div className="flex flex-wrap gap-2 mb-4">
        {technologies.map(function (tech, index) { return (<span key={index} className="bg-muted px-3 py-1 rounded-full text-xs text-muted-foreground">
            {tech}
          </span>); })}
      </div>
    </div>
    ...
    <div className="mt-4">
      <ul className="space-y-4 text-muted-foreground">
        {points.map(function (point, index) { return (<li key={index} className="flex items-start">
...
var Projects = function () {
    ...
    return (<section id="projects" className="py-20 bg-primary">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <SectionTitle number="04" title="Projects"/>
        
        <motion.div ref={sectionRef} initial="hidden" animate={controls} variants={fadeIn}>
